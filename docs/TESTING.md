...
| `detect_tech_tags`, `detect_requirements`, `parse_salary`, `extract_job_facets` (`scripts/patterns.py`) | `tests/test_patterns.py` | B3/B4/B5 job-facet detection: canonical tech tags from JD text (Java ≠ JavaScript, "React Native" not double-counted as "React"); **rejects ambiguous prose** — bare "go"/"spark"/"spring"/"rust" as ordinary words must not tag a language; visa-sponsorship / degree / relocation only when the text is explicit (negative statement wins over positive; silent text → key absent, never `False`); **`min_years_experience`** — lower bound of a range, "N years ago" prose rejected, no number → absent; **`languages_required`** — spoken languages with a fluency cue, Go/Rust/Swift never counted as spoken languages, English excluded, a bare country adjective ignored; `parse_salary` needs a currency-marked two-ended range and rejects junk (single number, >10× spread, min>max, out-of-bounds), infers `period` from magnitude when unstated |
| `keywordGap.ts` ↔ `patterns.py` parity (Lane R2) | `tests/test_patterns.py` | The site's résumé keyword-gap check mirrors `TECH_TAG_PATTERNS` / `detect_requirements` in TypeScript; the test parses the tag list out of `site/src/lib/keywordGap.ts` and asserts it matches `patterns.TECH_TAG_PATTERNS` (names **and** order) and that the three requirement keys are still checked — same drift guard as `formatSalaryShort` |
...
| `fetch_bamboohr_jobs`, `fetch_bamboohr_job_detail` | `tests/test_public_sources.py` | List+detail two-call mapping — the detail call only fires for the software-filtered posting (the fake `fetch_json` raises on any other URL, catching a would-be N+1 over every row); region before the `(Remote)` tag; description → B3/B4/B5 facets; empty board → no rows, no raise |
| `fetch_freshteam_jobs` | `tests/test_public_sources.py` | HTML-scrape regex mapping (no JSON API exists for this ATS) — anchor/title/location/remote-flag extraction, non-software titles filtered, region before the `(Remote)` tag, short excerpt → facets; empty board (no anchors) → no rows, no raise |
...
| `fetch_pinpoint_jobs` | `tests/test_public_sources.py` | `/postings.json` bare-list mapping; `_pinpoint_location` field probing; region resolved from location *before* the `(Remote)` suffix (a Dubai role stays `mena`, not `remote`); non-software-title filtering |
| `fetch_workable_jobs` | `tests/test_public_sources.py` | Widget-endpoint `{name, jobs:[…]}` mapping; `locations[]` → multi-location `<details>`; region resolved before the `(Remote)` suffix; `telecommuting` remote flag; facets from the HTML `description`; empty board → `[]`, no raise |
...
| `load_extra_job_boards` | `tests/test_public_sources.py` | YAML-by-hand parsing of `ashby:`/`smartrecruiters:`/`pinpoint:`/`workable:` sections (bare token → `<token>.pinpointhq.com`; dotted token kept as-is) |
...
| `load_extra_job_boards` (incl. hand-seeded `greenhouse:`/`lever:` sections) | `tests/test_public_sources.py` | Parses all sections (`ashby`/`smartrecruiters`/`greenhouse`/`lever`/`pinpoint`/`workable`/`workday`) from config |
