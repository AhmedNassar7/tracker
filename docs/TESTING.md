...
| `load_extra_job_boards` (incl. hand-seeded `greenhouse:`/`lever:` sections) | `tests/test_public_sources.py` | Parses all sections (`ashby`/`smartrecruiters`/`greenhouse`/`lever`/`pinpoint`/`workday`) from config |
