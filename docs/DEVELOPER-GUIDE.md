...
- **A dead-link false positive**: `check_url_alive()` only trusts a `GET`-confirmed 404/410; everything else (403, timeouts, DNS errors) is treated as "assume alive." If a posting you know is dead isn't getting archived, that's why — see the docstring on `check_url_alive` in `scripts/fetch.py`.
