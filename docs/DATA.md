...
| `level`, `region`, `role_type` | enum | Job-only. `region` label note: the site shows `emea` as **"Europe"**, not "EMEA" — `detect_region` tests `mena` first so Middle-East/Africa never lands in `emea` |
