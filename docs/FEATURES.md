...
2. **Sort** — four named modes with a one-line explanation each: **Top companies** (`companyTier()`, `lib/companyTiers.ts`, FAANG-first — the neutral default), **Newest** (by age), **Relevance** (disabled until you've saved preferences), **Best for you** (disabled until your profile has skills or targets). *Relevance* ranks by `scoreOpportunity()` (`lib/preferences.ts`) against your **saved filter** + two ranking-only knobs in a "Tune ranking" popover (keyword boost, hide companies). *Best for you* ranks by `scoreJobForProfile()` (`lib/profileMatch.ts`) against your **whole `/profile`** — skills ↔ `tech_tags` (canonical vocab), targets, salary floor, visa, degree, derived years-of-experience vs `min_years_experience` — and each row gets a `🎯 N% match` pill with reason / gap chips (`OpportunityTable`'s `MatchChips`). Both partition **flat contradictions** below a *"N less-relevant roles — show anyway"* toggle. The *Best for you* score is frozen onto `TrackedApplication.matchScore` when you Track a role, and the personal dashboard shows the distribution of those frozen scores (Strong ≥70 / Fair / Weak) with an average and a nudge when you're tracking more weak matches than strong. **Default sort**: a returning visitor whose profile has skills or targets lands on *Best for you*; failing that, on *Relevance* if a saved filter exists; otherwise *Top companies*. All three defaults are a *smart default* — never written to storage, so an explicit sort click always wins and the default follows the current profile/filter state.
...
- **`<StoryStrip>`** as a footer below the list (a top-border `<section>`, "This week's hiring trends") — `data/story-cards.json` cards (`fetchStoryCards()`), clicking one applies its pre-baked `filter`. **Strict division of labour with the hero: the hero owns every count** (total + the Internships / New-grad / Remote / Hackathons chips); the strip owns **trends and rankings and carries no raw counts at all** — `build_story_cards()` emits only level-*trend* cards ("Internships up 12% this month", count-free, and only when there's a month-old snapshot to compare against), a top-3-companies card, and a top-2-regions card. Nothing here can repeat or contradict a hero number.
...
- **Footer** (`Layout.astro`, every page) — two columns: what the site is + a GitHub source link + a `♥ Sponsor` link (github.com/sponsors/AhmedNassar7); and the five RSS feed links (job alerts, no account). Replaces the standalone `RssFeedLinks` box.
...
**Navigation (`Layout.astro`):** four top-level entries — **Jobs** (`/`), **Today** (`/today`), **Dashboard** (`/dashboard`), **My Workspace** (`/profile`). "My Workspace" covers the three local-only personal tools, which share a sub-tab bar (`WorkspaceNav.astro`): **Profile** (`/profile`), **Applications** (`/applications`), **Résumé check** (`/toolkit/resume`). Each keeps its own URL (shareable, bookmarkable); the sub-nav just presents them as one area. The nav + sub-tab links carry `data-astro-prefetch="viewport"` (config `prefetch: { prefetchAll: false }`), so switching between the three personal pages fetches the next document before you click — only island hydration remains as a perceptible cost. *(A standalone `/mena` view and a `/resources` page were both built and then removed 2026-09-10 — the first duplicated the Region filter, the second was static link lists that sat oddly beside a live job feed; `data/resources.md` stays as the hand-curated doc, linked from the README.)*

**`/today` (`TodayDigest.tsx`, C8):** a once-a-day glance, built entirely from local state — the hourly `site-index.json`, the saved profile, the "new since last visit" set (`visitHistory.ts`, read-only here — only the Jobs page advances it), and the tracked-application store. Three sections: **New for you** (roles new since your last visit, scored with `scoreJobForProfile` and filtered to `score ≥ 40 && !contradicts`, top 6 — or just the newest 6 with no profile), **Top picks for you** (best-scoring untracked jobs not already in "new", top 5, profile only), and **Your pipeline** (a one-line bookmarked / in-progress / offers count with a link to the tracker). Greeting + date header, an "Average N% / fill your profile" nudge, and context-aware empty states. No network beyond the same `site-index.json` fetch the main list makes.
