...
**How it works:** `OpportunityBrowser.tsx` is the main island — a `SnapshotHero` (live count, freshness, four one-tap shortcut chips), a collapsible `PreferencesPanel` (level/region/work-type/keywords/hide-company, stored in `localStorage`, never uploaded), a **Top companies / Newest / Best match** sort — *Top companies* is the default: it ranks by `companyTier()` (`lib/companyTiers.ts`, the same FAANG-first order as the pipeline) so the best-known names lead regardless of collector layer; *Best match* ranks by `scoreOpportunity()` (`lib/preferences.ts`) with "why it matched" chips — the `FilterBar`, and `OpportunityTable` (per-kind columns; `CompanyAvatar` shows a real favicon only for companies whose domain is hand-verified in `lib/companyLogos.ts`, trying two keyless providers (DuckDuckGo, then Google `faviconV2`) since neither covers every domain — for hackathon/event rows it instead derives the favicon from the row's own organiser URL, which can't be the wrong entity — else generated initials, never a guessed logo). The `FilterBar` covers kind / level / region / work-type / country plus the B3/B4/B5 facets — an "Any tech" tag dropdown and **🛂 Visa sponsorship** / **No degree required** toggles (explicit-only) — and `OpportunityTable`'s `FacetChips` shows the tech tags and a 🛂 / No-degree / pay-range chip row under each job (see "Job-facet detection" above). A **"Verified open · checked Xm ago"** badge (`LivenessBadge`, positive-only) appears on jobs whose `liveness` is `verified` — see "Dead-link detection" above. `<BrowseEveryRole>` renders the `kind:"board"` aggregate-links rows (Google / Meta / Microsoft / Apple / MENA majors) as a separate chip strip above the list — see "Dead-simple config extension points" below. Saved searches (`lib/savedSearches.ts`) persist named filter combos as chips. A **`<StoryStrip>`** between the hero and the list shows the 3–4 auto-generated cards from `data/story-cards.json` (`fetchStoryCards()`); clicking one applies its pre-baked `filter`. **"New since your last visit"** (`lib/visitHistory.ts`) diffs the current id set against the ids seen on the previous visit — a banner with a "show only new" toggle; the baseline re-writes on every load so a same-session reload correctly shows zero new. **"Not interested"** (`lib/dismissed.ts`) is the inverse: a per-row *Not interested* link hides an opportunity for good (a capped, most-recent-3000 id list in `localStorage`); a `N hidden — Review hidden / Clear all` line lets the viewer re-open the hidden set (rows render muted with an *undo*) or wipe it. Both are opportunity-only — `kind:"board"` rows never participate. `<ClientRouter />` gives native cross-page view transitions. The results list is **paginated** (`Pagination.tsx`, `PAGE_SIZE` 50) — a first/last + current±1 page bar with a "Showing 51–100 of 3,318" readout, not an ever-growing "load more" list; any filter/sort/preference change resets to page 1 and scrolls up.
