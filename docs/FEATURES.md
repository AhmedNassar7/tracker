...
- **Per-search webhook push** (Lane C4, `savedSearches.ts` `webhookUrl` field, the 🔗 button on each saved-search pill) — paste your own Discord/Slack/Telegram URL (stored only in this browser); `pushWebhookMatches` (`lib/notifications.ts`, run from `OpportunityBrowser` whenever fresh data loads, rate-limited to ~once per 6h) POSTs new matches for that search straight to it. Uses `mode: "no-cors"` deliberately — a JSON POST with `Content-Type: application/json` triggers a CORS preflight that most incoming-webhook endpoints (Slack's, notably) never answer, which would silently drop the request entirely; a `no-cors` "simple request" (no preflight) is what actually gets delivered, at the cost of never being able to confirm it server-side. Telegram has no generic webhook concept, so a pasted `sendMessage` URL just gets the text appended as a query param. Delivery is always best-effort and unconfirmed — the UI says so. *(A separate "🔔 Turn on alerts" browser-notification toggle — Lane C7 — was built alongside this and then **removed**: users didn't know what it meant, what it did, or when it would fire, and it duplicated a job the webhook already does more concretely — a URL you control, tied to one named search, no ambiguous permission prompt. `checkAndNotify` was replaced by the narrower `pushWebhookMatches`.)*
