...
**Navigation (`Layout.astro`):** three top-level entries — **Jobs** (`/`), **Dashboard** (`/dashboard`), **My Workspace** (`/profile`). Clean split: **`/dashboard` = the *global* picture** (region / level / source mix over everything published — `GlobalDashboard` — plus "this week's hiring trends" — `DashboardStory`); **My Workspace = *your* stuff**, three local-only tools sharing a sub-tab bar (`WorkspaceNav.astro`): **Profile** (`/profile`), **Applications** (`/applications` — the tracked-application board *and*, below it, `PersonalDashboard`: your funnel, stage timings, and frozen-match-score quality — moved here from the dashboard 2026-09-10 so all your-data lives in one place), **Résumé check** (`/toolkit/resume`). Each keeps its own URL (shareable, bookmarkable); the sub-nav just presents them as one area. The nav + sub-tab links carry `data-astro-prefetch="viewport"` (config `prefetch: { prefetchAll: false }`), so switching between the three personal pages fetches the next document before you click — only island hydration remains as a perceptible cost. *(Standalone `/mena`, `/resources`, and `/today` pages were each built and then removed 2026-09-10 — `/mena` duplicated the Region filter, `/resources` was static link lists beside a live feed, and `/today`'s "new since last visit" was near-always empty on the visit that opened it (the Jobs page advances last-visit) while its "top picks" and "pipeline" just restated the Best-match sort and the dashboard. `data/resources.md` stays as the hand-curated doc, linked from the README.)*
