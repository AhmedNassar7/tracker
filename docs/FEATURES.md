...
`detect_region` buckets a location into the **macro-region tier**: `north_america` / `latam` / `europe` / `mena` / `apac` / `remote` / `unknown`. This is a taxonomy, not a hybrid — the US and Canada are `north_america` (per-country granularity is the separate `country` field / Country filter), and South America is part of `latam`, so there's no `south_america`. **`mena` (Middle East & Africa) is checked before `europe`** so a Gulf, Levant, North-Africa, or sub-Saharan-tech-hub location (Dubai, Riyadh, Cairo, Amman, Tel Aviv, Lagos, Nairobi, Cape Town, Istanbul, …) resolves to its own region instead of being folded into Europe. It's in `WANTED_REGIONS`, so the curated layer keeps these roles. Region enums in `config/job-entry.schema.json`, `config/public-entry.schema.json`, and `config/site-index.schema.json` all carry the seven values (older data may still carry the pre-2026-09-06 `us`/`canada`/`emea`).

On the site, `geo.ts` `regionForItem()` resolves the bucket client-side — the pipeline's own value (mapped through `REGION_ALIASES` for legacy data) → an explicit "remote" location → derived from the resolved country (`REGION_BY_COUNTRY`) → named in the location text → `unknown`. So `apac`/`latam` show up in the Region filter immediately, before the pipeline re-runs `detect_region` over the public layer. The Region `<MultiSelect>` is **data-driven** (`availableRegions` in `OpportunityBrowser`) — it only lists buckets present in the loaded data, so `unknown` disappears from the filter once classification is complete instead of sitting there as a dead option. `filtersFromSearchParams` and `preferences.readPrefFilter` both remap `us`/`canada`/`emea` → the new values so old shared URLs and saved preferences keep working.
