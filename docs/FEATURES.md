...
1. **`FilterBar`** — the *only* place facets live: kind tabs · search · **multi-select** Level / Region / Work-type / Country / Tech (a reusable `MultiSelect` popover; Country + Tech are searchable) · explicit-only 🛂 Visa / No-degree toggles. Every array facet serialises to a comma-joined URL param, so a filtered view is a shareable link. A **"★ Save as my preferences"** button (next to *Clear filters*) stores the current filter as your preference.
...
- **`<StoryStrip>`** between the hero and the list — the 3–4 auto-generated cards from `data/story-cards.json` (`fetchStoryCards()`); clicking one applies its pre-baked `filter` (partial `FilterState`, array-facet shape).
...
- **`OpportunityTable`** — per-kind columns; `LivenessBadge` ("Verified open · checked Xm ago", positive-only); `FacetChips` (tech tags + 🛂 / No-degree / pay-range under each job); a country flag in the location cell; `CompanyAvatar` (real favicon only for a hand-verified domain in `lib/companyLogos.ts` via two keyless providers, else initials — never a guessed logo).
