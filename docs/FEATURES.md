...
- **Footer** (`Layout.astro`, every page) — a one-line "what this is", then a tidy `<dl>`: an **RSS** row (the five feed links) and a **Project** row (Source & docs · ♥ Sponsor → github.com/sponsors/AhmedNassar7). Replaces the standalone `RssFeedLinks` box; the "Browse directly" company list that briefly lived here was removed as noise.
