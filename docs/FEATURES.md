...
2. **Sort** — four named modes with a one-line explanation each: **Top companies** (`companyTier()`, `lib/companyTiers.ts`, FAANG-first — the neutral default), **Newest** (by age), **Relevance** (disabled until you've saved preferences), **Best for you** (disabled until your profile has skills or targets). *Relevance* ranks by `scoreOpportunity()` (`lib/preferences.ts`) against your **saved filter** + two ranking-only knobs in a "Tune ranking" popover (keyword boost, hide companies). *Best for you* ranks by `scoreJobForProfile()` (`lib/profileMatch.ts`) against your **whole `/profile`** — skills ↔ `tech_tags` (canonical vocab), targets, salary floor, visa, degree, derived years-of-experience vs `min_years_experience` — and each row gets a `🎯 N% match` pill with reason / gap chips (`OpportunityTable`'s `MatchChips`). Both partition **flat contradictions** below a *"N less-relevant roles — show anyway"* toggle. The *Best for you* score is frozen onto `TrackedApplication.matchScore` when you Track a role. A returning visitor with saved preferences lands on *Relevance* automatically (until they pick a sort explicitly).
...
- **Footer** (`Layout.astro`, every page) — a three-column `<footer>`: what the site is + a GitHub source link; the five RSS feed links (job alerts, no account); and **"Browse directly"** — the `aggregate_links.yml` companies (Google / Meta / Microsoft / Apple / GitHub / Arm / Cisco / Splunk / Shopify + MENA majors) as plain links, parsed from the YAML at build time. Replaces the old on-list `<BrowseEveryRole>` strip and the standalone `RssFeedLinks` box.
...
**Navigation (`Layout.astro`):** four top-level entries — **Jobs** (`/`), **Dashboard** (`/dashboard`), **My Workspace** (`/profile`), and the repo link. "My Workspace" covers the three local-only personal tools, which share a sub-tab bar (`WorkspaceNav.astro`): **Profile** (`/profile`), **Applications** (`/applications`), **Résumé check** (`/toolkit/resume`). Each keeps its own URL (shareable, bookmarkable); the sub-nav just presents them as one area. *(A standalone `/mena` view and a `/resources` page were both built and then removed 2026-09-10 — the first duplicated the Region filter, the second was static link lists that sat oddly beside a live job feed; `data/resources.md` stays as the hand-curated doc, linked from the README.)*
