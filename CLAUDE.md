...
- **Curated layer** (`scripts/fetch.py`) — pulls Remotive, ArbeitNow, SimplifyJobs, speedyapply (SWE + AI), zapplyjobs, hanzili, and ambicuity. Every row is checked against `config/companies_allowlist.yml` (substring match, case-insensitive) — this is the strict, high-quality feed. Writes `data/jobs-global.json` / `data/jobs-global-archive.json` / `data/jobs-global-latest.md` / `data/stats.json` via `scripts/fetch_outputs.py`.
- **Public layer** (`scripts/public_sources.py`) — widens coverage via Greenhouse, Lever, and Workday (all **auto-discovered**: the first time one of these company's postings shows up from any source, its board gets polled directly on the next run — no config needed), plus Ashby/SmartRecruiters for companies listed by hand in `config/extra_job_boards.yml`, plus Devpost hackathons and Luma events. Writes `data/public-opportunities.json`/`.md` via `scripts/public_outputs.py`.
- `scripts/build_data_readme.py` loads both JSON outputs, merges them, and renders the two README files. It's the single source of truth for all human-facing text in this repo — badges, counts, table formatting, and the "How it works" prose all live in its `render_root_readme` / `render_data_readme` functions.
...
- `config/sources.yml` — documentation only, not read by any script. Keep it in sync by hand when adding/removing a source in `fetch.py` or `public_sources.py`.
- `data/resources.md` — hand-curated, not regenerated by the pipeline. Static career-resource links (interview prep, resume tools, etc.), separate from the live job data.
