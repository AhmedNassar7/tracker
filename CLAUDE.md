...
# Verify Greenhouse/Lever/Ashby/SmartRecruiters board tokens before adding them to
# config/extra_job_boards.yml (needs network; standalone, doesn't touch the pipeline):
python scripts/verify_extra_boards.py            # re-check every token already in the config
python scripts/verify_extra_boards.py --mena     # check the MENA/Gulf/NA candidate shortlist
python scripts/verify_extra_boards.py --europe   # check the Europe-HQ'd candidate shortlist
python scripts/verify_extra_boards.py greenhouse:swvl ashby:telda   # check specific ones
...
- **Curated layer** (`scripts/fetch.py`) — pulls Remotive, ArbeitNow, SimplifyJobs, speedyapply (SWE + AI), zapplyjobs, hanzili, ambicuity, DereC4, Lamiiine, plus three first-party company APIs hit directly (Amazon `amazon.jobs`, Netflix Eightfold, Apple `jobs.apple.com/api/v1/search` — keyless CSRF handshake) and Germany's Arbeitsagentur. Every row is checked against `config/companies_allowlist.yml` (substring match, case-insensitive) — this is the strict, high-quality feed. Amazon and Apple titles carry no level word, so both are in `UNKNOWN_LEVEL_SOURCES` (unlabelled titles pass through; `SENIOR_TITLE_RE` still drops senior/lead). Writes `data/jobs-global.json` / `data/jobs-global-archive.json` / `data/stats.json` via `scripts/fetch_outputs.py`.
- **Public layer** (`scripts/public_sources.py`) — widens coverage via Greenhouse, Lever, and Workday (normally **auto-discovered**: the first time one of these company's postings shows up from any source, its board gets polled directly on the next run — no config needed; all three also have a hand-seed section in `config/extra_job_boards.yml` for a company none of the curated sources ever link a board URL for), plus Ashby/SmartRecruiters/PinpointHQ/Workable for companies listed by hand there (PinpointHQ via the `pinpoint:` section — `fetch_pinpoint_jobs` polls `{host}/postings.json`, where `{host}` is `{token}.pinpointhq.com` for a bare token or a custom careers host if the token has a dot; added 2026-09-06 for the MENA push — Tabby, Money Fellows. Workable via the `workable:` section — `fetch_workable_jobs` does one keyless GET on `apply.workable.com/api/v1/widget/accounts/{slug}?details=true` (`{name, jobs:[…]}`, full HTML JD inline, `telecommuting` = remote flag, `locations[]` for multi-site); added 2026-09-07 for the MENA push — Foodics, Lucidya, Salla). Recruitee via the `recruitee:` section — `fetch_recruitee_jobs` does one keyless GET on `{slug}.recruitee.com/api/offers/` (`{offers:[…]}`, `status=="published"` only, `description`+`requirements` HTML inline, `remote` flag, `locations[]`); added 2026-09-10 — sahl (Cairo). Plus Devpost/Unstop/Devfolio/HackerEarth hackathons (their own keyless JSON APIs — the hackathons *pages* are client-rendered; HackerEarth via `fetch_hackerearth_hackathons` hitting `/chrome-extension/events/`, added 2026-09-07, small high-signal set strong on MENA/India), Luma events (filtered for tech/software relevance), and a hand-maintained list of tech/career conferences & summits in `config/events.yml` (past-dated rows auto-hidden). Writes `data/public-opportunities.json` via `scripts/public_outputs.py`.
...
- `config/extra_job_boards.yml` — Ashby/SmartRecruiters/PinpointHQ/Workable/Recruitee companies to poll (Recruitee via the `recruitee:` section, a bare `{slug}.recruitee.com` subdomain per line), plus `greenhouse:`/`lever:`/`workday:` sections for hand-seeding a company whose board would otherwise never get auto-discovered (auto-discovery only ever triggers once one of a company's postings organically surfaces through an *existing* curated fetcher — a company none of those ~17, mostly US/EU-focused, sources ever mention stays invisible indefinitely even with a real, live, publicly pollable board; this is exactly the gap that left MENA-region companies like Careem — 231 real Dubai/UAE postings on Greenhouse — and Thndr — Cairo, Egypt, on Ashby — undiscovered until verified and added by hand). The `greenhouse:`/`lever:`/`ashby:`/`smartrecruiters:` entries are a bare board token per line; **`workday:` entries are `Company Name | host | site`** (three fields — Workday has no single token, each tenant is its own subdomain plus a site path), verified with `curl -XPOST -d '{"limit":5,"offset":0,"searchText":"software engineer"}' https://<host>/wday/cxs/<tenant>/<site>/jobs`. **SmartRecruiters' API returns HTTP 200 with an empty result for *any* company slug, valid or not** — there's no way to verify a guessed token via the API alone, so don't add SmartRecruiters entries without an out-of-band way to confirm the slug is real. Ashby's API *does* 404 on an invalid token, so new Ashby boards can be verified directly: `curl https://api.ashbyhq.com/posting-api/job-board/<token>`. Same discipline applies to hand-seeded Greenhouse (`curl https://boards-api.greenhouse.io/v1/boards/<token>/jobs`), Lever (`curl https://api.lever.co/v0/postings/<token>?mode=json`), and Workable (`curl 'https://apply.workable.com/api/v1/widget/accounts/<slug>?details=true'` — 404s on a bad slug, but 200 + empty `jobs` for a real account with nothing open, so confirm the list is non-empty) tokens — confirm real postings exist, not just a non-404 response, before adding one. `scripts/verify_extra_boards.py` automates this "confirm by hand" step (re-checks every token in the file, or `--mena` / `--europe` / `platform:token` args for candidates) and prints a ready-to-paste list of the confirmed ones; a wrong token is otherwise *safe but silent* — the fetcher logs a `[WARN]` and contributes zero rows, so nothing else breaks, but a valid-token-wrong-company would publish mislabeled jobs, which is what the verifier's sample-titles output catches.
