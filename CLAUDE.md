...
- **Public layer** (`scripts/public_sources.py`) — widens coverage via Greenhouse, Lever, and Workday (normally **auto-discovered**: the first time one of these company's postings shows up from any source, its board gets polled directly on the next run — no config needed; all three also have a hand-seed section in `config/extra_job_boards.yml` for a company none of the curated sources ever link a board URL for), plus Ashby/SmartRecruiters/PinpointHQ for companies listed by hand there (PinpointHQ via the `pinpoint:` section — `fetch_pinpoint_jobs` polls `{host}/postings.json`, where `{host}` is `{token}.pinpointhq.com` for a bare token or a custom careers host if the token has a dot; added 2026-09-06 for the MENA push — Tabby, Money Fellows), plus Devpost/Unstop/Devfolio hackathons (their own JSON APIs — the hackathons *pages* are client-rendered), Luma events (filtered for tech/software relevance), and a hand-maintained list of tech/career conferences & summits in `config/events.yml` (past-dated rows auto-hidden). Writes `data/public-opportunities.json` via `scripts/public_outputs.py`.
