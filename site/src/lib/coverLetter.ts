...
export function buildCoverLetter(input: CoverLetterInput, p: Profile): CoverLetter {
  ...
  const rawContribution =
    role?.bullets.find(Boolean)?.replace(/^[•\-*\s]+/, "").trim() || "[a concrete thing you shipped or owned there]";
  ...
  if (input.template === "referral") {
    ...
    const win = quantifiedBullet(p) || "[a result you're proud of — with a number]";
    paragraphs.push(
      `${win}. That's the kind of work I want to keep doing, which is why I'm applying for the ${title} role at ${company}.`,
    );
