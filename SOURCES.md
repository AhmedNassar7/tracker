...
- **Non-standard ATSes surfaced by the MENA push** — the region's employers cluster on ATSes the pipeline doesn't read yet. Each has a public keyless JSON API and would follow the same pattern as the existing 5 fetchers; `scripts/verify_extra_boards.py` already has a checker for each (`platform:token`), so verify the endpoint shape before building:
  - **Workable** — `apply.workable.com/api/v3/accounts/{sub}/jobs`. The Egyptian startup scene (Elmenus, Breadfast, Rabbit, …) is mostly here.
  - **PinpointHQ** — `{company}.pinpointhq.com/postings.json` (or a custom domain like `careers.moneyfellows.com`). Seen: **Tabby** (`tabby.pinpointhq.com`), **Money Fellows** (`careers.moneyfellows.com`, Cairo fintech, "powered by pinpoint").
  - **BambooHR** — `{company}.bamboohr.com/careers/list` (`{"result": […]}`). Seen: **Instabug / "Luciq"** (`instabug.bamboohr.com`).
  - **Recruitee** — `{company}.recruitee.com/api/offers/` (`{"offers": […]}`). Seen: **MoneyHash** (`moneyhash.recruitee.com`, 0 open when checked).
  Added so far via routes we *do* support: **Mercor** (`ashby:mercor`, 96 jobs), **Bosta** (`lever:Bosta`, Cairo).
