...
- **PinpointHQ** (`*.pinpointhq.com`) — a 6th ATS seen in the MENA push (Tabby's board is `tabby.pinpointhq.com`). No fetcher yet; it has a public postings API (`/postings.json` per company), so it's a plausible add alongside Workable (Lane M3). Tracked in `config/extra_job_boards.yml`'s MENA notes.
- **BambooHR** (`<company>.bamboohr.com/careers`) — a 7th ATS. **Instabug (rebranded "Luciq")** runs `instabug.bamboohr.com`; lots of small/mid companies do. Public keyless JSON at `/careers/list` (`{"result": [...]}`). `scripts/verify_extra_boards.py` can check a subdomain (`bamboohr:instabug`); a real fetcher is a good broad add alongside Workable/Pinpoint (Lane M3 / backlog B2). Verified ✓ and added so far via the `ashby:` route: **Mercor** (`mercor.com/careers` → "Powered by ashby", 96 jobs).
